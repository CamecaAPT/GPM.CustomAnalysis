...
class CMapping
{
	...
	// Functions
	// --------------
	public bool bBuildComposition(float[] fMapSize, float fDelocalization, CAtom Atom, bool[] bShowElt)
	{
		...
		iLocalMem = new int[(iNbStep[0] + 2), (iNbStep[1] + 2), (iNbStep[2] + 2), 2];
		...
		else
		{
			Parallel.For(0, Atom.iMemSize, ii =>
			{
				...
				for (aa = Math.Max(iLocalBlocId[0] - iDeltaDeloc[0], 0); aa <= Math.Min(iLocalBlocId[0] + iDeltaDeloc[0], iNbStep[0] - 1); aa++)
				{
					...
					for (jj = 0; jj < 2; jj++)
						if (Atom.bEltId[ii, jj] < Atom.iNbElt)
						{
...
*/
	...
	public bool bBuildOptimization(float fAtomMapSize, int iSelectionType, CAtom Atom, bool[] bShowElt)
	{

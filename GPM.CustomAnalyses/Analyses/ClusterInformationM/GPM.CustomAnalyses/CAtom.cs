...
public class CAtom
{
	...
	public async void bInitMemory2(IIonData LocalIonData, IIonDisplayInfo ionDisplayInfo)
	{
		int i, j;
		mass.Clear();
		pos_init.Clear();
		...
		string[] requiredSections = new string[] { IonDataSectionName.Position, IonDataSectionName.Mass, IonDataSectionName.IonType };
		bool allSectionsAvailable = await LocalIonData.EnsureSectionsAvailable(requiredSections, null);
		Console.WriteLine(allSectionsAvailable);
		if (allSectionsAvailable)
		{
			mass = LocalIonData.ReadSectionToArray<float>(IonDataSectionName.Mass).ToList();
			pos_init = LocalIonData.ReadSectionToArray<Vector3>(IonDataSectionName.Position).ToList();
			name = LocalIonData.ReadSectionToArray<byte>(IonDataSectionName.Position).ToList();
		}
		else
		{
			Console.WriteLine("Error load x,y,z,mas,type");
		}


		multiplicity = await LoadAtoMemory<int>(LocalIonData, IonDataSectionName.Multiplicity);

		requiredSections = new string[] { IonDataSectionName.Voltage };
		allSectionsAvailable = await LocalIonData.EnsureSectionsAvailable(requiredSections, null);
		if (allSectionsAvailable)
		{
			vdc = LocalIonData.ReadSectionToArray<float>(IonDataSectionName.Voltage).ToList();
		}
		else
		{
			Console.WriteLine("No Multiplicity Voltage");
		}

		requiredSections = new string[] { IonDataSectionName.DetectorCoordinates };
		allSectionsAvailable = await LocalIonData.EnsureSectionsAvailable(requiredSections, null);
		if (allSectionsAvailable)
		{
			pos_det = LocalIonData.ReadSectionToArray<Vector2>(IonDataSectionName.DetectorCoordinates).ToList();
		}
		else
		{
			Console.WriteLine("No Multiplicity Detector Coordinates");
		}


		//LocalIonData.AddSectionIfNotExists<float>(AddSectionContext.CreateOneToOne<float>("RandomizedMass", unit: "Da"));
